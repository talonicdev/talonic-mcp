...
- **Schema-typing footgun option 2: file API request.** Re-test on 2026-05-19 confirmed `type` is still absent from `talonic_search` `fieldMatches[]` and `fields[]` (schemas already expose it). File the API request with engineering to add `type` to both arrays on the search endpoint, mirroring the per-field type already returned on schema definitions. MCP-side outputSchema extension is one commit once the API ships.
...
2. **Surface field type in `talonic_search` and `talonic_list_schemas` outputs** so agents can check `field.type === 'number'` before constructing a `gt`/`lt` condition. **Status 2026-05-19 (re-tested against production via `scripts/probe-field-type.mjs`):** `talonic_list_schemas` already returns per-field types at `schemas[].definition.properties[<field>].type` (JSON Schema standard — verified values include `"string"`, `"number"`, etc.); no MCP-side work needed there. `talonic_search` still does not return `type` on either `fieldMatches[]` (keys: `resolvedFieldId, displayName, matchedValue, documentCount, filterable`) or `fields[]` (keys: `id, canonicalName, displayName, documentCount, filterable`). **Engineering coordination required**: file an API request to add `type` (or `data_type`) to both `fieldMatches[]` and `fields[]` entries on the `/v1/documents/search` response, matching the per-field type the schemas endpoint already exposes. Once landed, the MCP side is a one-commit outputSchema extension plus a one-line nudge in the agent decision guide ("check `type === 'number'` before constructing a numeric operator"). Stop-gap workaround for agents today: call `talonic_list_schemas` first to build a fieldName → type map, then guard the filter; costs one extra API round-trip per session.
