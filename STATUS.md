...
1. **`talonic_extract` via drag-and-drop in Claude.ai is bottlenecked by Claude.ai's tool-call argument size cap.** Diagnosed 2026-05-06; **measurement-refined 2026-05-20**. The cap is real but ~30× larger than originally documented: **~43,000 base64 chars / ~32 KB decoded payload**, not sub-1KB. Verified empirically against the hosted MCP via env-gated debug tools deployed in commit `3f7fb98`. The truncation is silent and clean — the wire delivers a properly-padded base64 prefix, agents never see they've been truncated, the Talonic API receives a stub document and returns null fields. Files under ~32 KB pass through `file_data` intact; real invoices/contracts (typically 100 KB+) do not. Workaround in Claude.ai today: `file_url` or `document_id`. Local-stdio installs (Claude Desktop, Cursor, Cline, Continue, Cowork) have no parameter cap and work correctly via `file_data`. **Status 2026-05-27 (investigation closed):** all scenarios complete. Scenario 4 confirmed the sandbox cannot PUT out-of-band (`*.amazonaws.com` → HTTP 403 "Host not in allowlist"), so canonical presigned-PUT is dead for Claude.ai. **Decision: browser-handoff upload.** The debug instrumentation has been removed from the codebase. Full findings + implementation plan for internal teams: `docs/superpowers/specs/2026-05-27-claude-file-upload-report.md`; backing detail in `docs/superpowers/specs/2026-05-20-presigned-upload-urls-design.md`.
