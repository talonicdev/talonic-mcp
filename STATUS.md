...
> **🔄 ACTIVE WORK 2026-05-20 — Pre-signed upload URLs investigation in progress.**
> Debug instrumentation is **deployed live** on `mcp.talonic.com` (commit `3f7fb98`, gated by `TALONIC_DEBUG_TOOLS=1` env var which is currently set in Railway). Three diagnostic MCP tools (`talonic_debug_echo`, `talonic_debug_request_upload_url`, `talonic_debug_check_upload`) plus an S3 test bucket are operational. Measurements 2026-05-20 disproved the long-documented "~1 KB tool-call argument cap" — **the real cap is ~32 KB decoded payload / ~43 KB base64 string** on Claude.ai's hosted connector pipeline. The project is still warranted; only Scenario 4 (direct PUT from sandbox) remains to be run before locking the design. **Resume point:** `docs/superpowers/specs/2026-05-20-presigned-upload-urls-design.md`. **Do not run the cleanup revert yet.**
...
- **Pre-signed upload URLs.** Architectural fix for Claude.ai's tool-call argument-size cap on `file_data`. **Active investigation as of 2026-05-20.** Debug instrumentation deployed (commit `3f7fb98`, env-gated by `TALONIC_DEBUG_TOOLS=1`); empirical measurements have revised the cap from "~1 KB" to "~32 KB decoded / ~43 KB base64". Scenarios 1, 2, and 2-diagnostic complete. Scenario 4 (direct PUT from sandbox to S3) still pending. See `docs/superpowers/specs/2026-05-20-presigned-upload-urls-design.md` for full investigation state, the Scenario 4 prompt to copy-paste into Claude.ai, and the cleanup checklist.
...
1. **`talonic_extract` via drag-and-drop in Claude.ai is bottlenecked by Claude.ai's tool-call argument size cap.** Diagnosed 2026-05-06; **measurement-refined 2026-05-20**. The cap is real but ~30× larger than originally documented: **~43,000 base64 chars / ~32 KB decoded payload**, not sub-1KB. Verified empirically against the hosted MCP via env-gated debug tools deployed in commit `3f7fb98`. The truncation is silent and clean — the wire delivers a properly-padded base64 prefix, agents never see they've been truncated, the Talonic API receives a stub document and returns null fields. Files under ~32 KB pass through `file_data` intact; real invoices/contracts (typically 100 KB+) do not. Architectural fix is still pre-signed upload URLs (browser-handoff or canonical PUT). Workaround in Claude.ai today: `file_url` or `document_id`. Local-stdio installs (Claude Desktop, Cursor, Cline, Continue, Cowork) have no parameter cap and work correctly via `file_data`. **Status 2026-05-20:** Scenarios 1, 2, and 2-diagnostic complete. Scenario 4 (direct PUT from sandbox to S3) pending — the result determines whether the fix is browser-handoff only or a hybrid with direct PUT. See `docs/superpowers/specs/2026-05-20-presigned-upload-urls-design.md` for the full investigation, the Scenario 4 prompt, and the cleanup checklist.
