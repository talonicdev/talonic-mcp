...
`@talonic/mcp` is at **0.1.52** on npm, `@talonic/node` is at **0.1.23** on npm. **All 9 MCP tools and 2 resources** are stable in production via both stdio and the hosted endpoint. OAuth 2.1 connector flow on Claude.ai is live and verified. The hosted MCP endpoint serves Streamable HTTP at both `/` and `/mcp`; `mcp-publisher` and `gh release create` are wired into the publish workflow via GitHub OIDC; `talonic_filter` surfaces the API `warnings` array and the preventive `dataType` guard closes the schema-typing footgun end-to-end. **The Claude.ai file-upload blocker is solved and verified live (2026-06-03):** the 9th tool, `talonic_request_upload`, implements the browser-handoff flow (see the banner above). A ChatGPT Apps SDK extraction-result widget also ships (0.1.45). Everything is resolved or operational; remaining items are optional polish (handoff latency, early-extract race hardening).
...
| server.json version         | 0.1.52                                                                                                                                                                                                                 |
| npm published version       | 0.1.52 (auto-bump pipeline; in sync with the Registry)                                                                                                                                                                 |
...
| Tests                       | 89 pass (covers all 9 tools incl. `talonic_request_upload`, the Apps SDK widget + tool-annotation contract, and `/` + `/mcp` HTTP routing; symlink tests skip when `dist/` is older than `package.json`; verified 2026-06-03) |
| Format check                | clean                                                                                                                                                                                                                  |
| Typecheck                   | clean                                                                                                                                                                                                                  |
| Build                       | clean                                                                                                                                                                                                                  |
...
| Tools                       | 9 stable: `talonic_extract`, `talonic_request_upload`, `talonic_search`, `talonic_filter`, `talonic_get_document`, `talonic_to_markdown`, `talonic_list_schemas`, `talonic_save_schema`, `talonic_get_balance`        |
