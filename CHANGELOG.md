...
- **Apps toolset** (`src/tools/apps.ts`) — fifteen management tools over the platform's `/v1/apps` surface: list/get/create apps, save + publish draft versions, enable/disable (kill switch), run (with `dry_run` and `idempotency_key`), run inspection (projection + append-only journal), sealed decision-record export, and Human Reviews (list, raise, resolve with the two-part resolution + feedback payload), plus precedents (v1.1 server-side).
- **Dynamic per-app tools** (`src/tools/app-tools.ts`) — every enabled Talonic App registers as its own `app_<slug>` tool with the app's input JSON Schema converted to the tool schema (`src/tools/json-schema-to-zod.ts`, 2020-12 subset with hard limits: depth 6, 100 properties, 32 KB; documented permissive fallback). Token-scoped catalog with per-token 5-minute cache and ETag revalidation. Stateless hosted transport recomputes the tool list per request by design; stdio sessions poll (`TALONIC_APPS_REFRESH_MS`, default 60000) and emit `tools/list_changed`.
