...
**Full agent orientation lives in [`AGENTS.md`](AGENTS.md).** Read it first — it covers the repo map, the thirty-six tools, the browser-handoff upload architecture, the CI/publish chain, and the common workflows. This file repeats only the two things you must not get wrong, plus the commands you'll use every session.
