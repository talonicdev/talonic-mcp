...
// → structured JSON with per-field confidence scores`,
  ...
  },
  {
    ...
    content: [
      ...
      { type: "heading", level: 3, id: "builder-toolset", text: "Builder toolset" },
      {
        type: "paragraph",
        text: "Ten further tools cover building an app to a TESTED state and maintaining it: `talonic_get_app_logic` and `talonic_get_app_fields` read the draft and the fields rules can be grounded in; `talonic_interpret_app_rules` compiles rule sentences into checkable rules, accepting plain-language `guidance` for stuck rules; `talonic_propose_app_edit` / `talonic_apply_app_edit` turn a natural-language change request into typed, reviewable edit operations; `talonic_save_app_acceptance` stores labeled ground truth (per-subject verdict expectations, or an expected run decision) with a tolerance, and `talonic_check_app_acceptance` scores any run against it by pure ledger comparison — the machine-checkable definition of done a builder agent iterates toward and a maintainer re-checks for drift; `talonic_get_run_verdicts` reads a batch run's per-subject results matrix; `talonic_interview_app_review` holds one turn of the natural-language review interview, with resolution values validated against the review's input contract server-side.",
      },
