...
// })
{
      ...
      { type: "heading", level: 3, id: "run-spec-response", text: "Response shape" },
      {
        ...
        params: [
          ...
          { name: "spec_id", type: "string", description: "The Spec that ran." },
          {
            name: "spec_name",
            type: "string|null",
            description: "The Spec's display name, when the backend returned one; null otherwise.",
          },
          {
            name: "status",
            type: "string",
          ...
          {
            name: "enqueued_documents",
            type: "number|null",
            description:
              "Documents enqueued for processing on the `document_ids` route; absent on the `file_urls` route.",
          },
          {
            name: "appended",
            type: "boolean",
            description:
              "Whether this run appended to the Spec's existing pipeline (`pipeline_mode: 'append'`); present on the `document_ids` route only.",
          },
          {
            name: "documents[]",
            type: "array",
          ...
          },
          {
            ...
            type: "string|null",
            description:
              "Informational message from the API, e.g. 'Pipeline created and queued for processing.'; null when not provided.",
          },
  ...
  },
  {
    ...
    content: [
      {
        type: "paragraph",
        text: "Poll a Spec run started by `talonic_run_spec`: a normalized `status`, document-level progress, and — for pipeline runs — per-phase progress. Pass exactly one of `run_id` or `pipeline_id` — poll with `pipeline_id` when `run_kind` is `pipeline`, or with `run_id` when it is `run`.",
      },
        ...
        params: [
          {
            ...
            type: "string",
            description:
              "From a `run_kind: 'run'` envelope, alone — or accompanying `pipeline_id` to scope that pipeline's results to one submission.",
          },
          {
            name: "pipeline_id",
            type: "string",
            description:
              "From a `run_kind: 'pipeline'` envelope. Optionally pass `run_id` alongside it to scope results to one submission within the pipeline.",
          },
