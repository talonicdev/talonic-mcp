...
**Goal:** Local `main` merges `origin/main` (PR #22 decision tasks published as 0.1.77, PR #23 registry-wait CI fix published as 0.1.78), the seven decision-task tools get the same ChatGPT widget parity, manifest, mirror and website coverage as the other 29 (→ 36), and the sub-project 2 final review's remaining findings are fixed — leaving a branch that fast-forwards onto `origin/main` and passes every lock at 36.
...
- [ ] **Step 2: Resolve, file by file (keep BOTH sides everywhere):**
  ...
  - `CHANGELOG.md`: keep our `[Unreleased]` bullets AND their decision-task bullet, all under `[Unreleased]` ### Added (repo practice: the auto-bump never cuts headings, 0.1.53–0.1.78 all sit under Unreleased — do NOT invent a `## [0.1.77]` heading). Order: their decision-task bullet directly after our Specs/Run/Ask bullet. In `[Unreleased]`: change "Widget parity at 22 tools" → "Widget parity for every public tool (36 with this release)" and "describes all 22 tools" → "describes all public tools"; replace "(7 new, 29 public)" with "(7 new, 36 public)"; add the bullet `- **Decision-task widgets.** The seven `talonic_*_decision_task` tools shipped in 0.1.77 get ChatGPT cards (worklist, claim bundle, package page, and lease/submit/release/fail metadata cards), manifest entries, mirror docs and website pages.` right after their decision-task bullet; and add directly under the `## [Unreleased]` heading the line `> 0.1.77 shipped the decision-task tools (PR #22) and 0.1.78 the registry-wait CI fix (PR #23), both 2026-09-22; the next push publishes 0.1.79.`
  ...
  - `package.json` / `server.json`: upstream's `0.1.78` (auto-merged; verify both files).
...
- [ ] **Step 4: Commit the merge** — `git commit` (merge commit message: `Merge origin/main (0.1.78: decision tasks + registry-wait CI) into main — union of both lineages; counts to 36` + trailer). This single commit may carry red locks; Task 2 turns them green in the same dispatch.
...
- [ ] Report: commits (both repos), test totals, preflight line, and the release order (talonic-mcp push → CI 0.1.79 → website pin bump → website push).
