...
Exempting them would reopen exactly the gap Hamlet asked to close, so the ruling is: **merge, then bring the seven to full parity**, and fold in the sub-project 2 final review's remaining findings. The release becomes **0.1.78**.
...
5. **Website:** merge `origin/main` (lockfile-only upstream commit), seven decision-task pages + registries + sitemap + mcp.json entries + hub prose at thirty-six; **push order** recorded: talonic-mcp first → CI publishes 0.1.78 → website `npm update @talonic/mcp` + lockfile commit → website push.
...
Full chain + preflight; `git status` clean; local `main` ahead of `origin/main` with no divergence (`behind 0`). Release 0.1.78 on Hamlet's go; website after the pin bump.
